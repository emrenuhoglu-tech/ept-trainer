...
// "Js" = maça vale (rank+suit), "JTs" = jack-ten suited (iki rütbe + s). İkinci karakter
// rütbe mi diye bakarak ayırt edilir.
...
// Kapalı kart — kitapta belirtilmemiş sokak kartı için.
...
// Suit'i belirtilmemiş genel el (JTs/AKo/AA) için temsili suit seçimi.
function glyphSuits(code: string): [string, string] {
  if (code.length >= 3 && code[2] === "s") return ["s", "s"]; // suited → aynı renk
  if (code.length >= 3 && code[2] === "o") return ["s", "h"]; // offsuit → farklı renk
  return ["s", "h"]; // çift → iki farklı suit
}

// Genel el kodu: "JTs" | "AKo" | "AA"
...
// Belirli kartlar veya el kodları içeren bir dizi. "Ah Kd 5c" (board) ya da "JTs".
