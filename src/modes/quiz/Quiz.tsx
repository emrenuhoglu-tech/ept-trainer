...
// Zayıf (opener, pozisyon) eşleşmeleri: yanlış yapılan 3-bet quiz kayıtlarından.
...
function RangeQuizBody() {
  ...
  if (!q) {
    return <div className="p-6 text-neutral-400">Quiz verisi yüklenemedi.</div>;
  }
  ...
  function answer(a: QAction) {
    ...
    recordResult({
      ...
      soru_ozeti: `${q.opener} açtı, sen ${q.position}, ${q.hand}`,
      sonuc: ok ? "correct" : "wrong",
      not: `doğru: ${ACTION_LABEL[q.correct]}`,
    });
  }
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-end text-sm">
        <span className="text-neutral-400">
          Seans {score.ok}/{score.total} · 🔥 {stats.streak} gün
        </span>
      </div>
      ...
      <div key={`${q.opener}-${q.position}-${q.hand}`} className="card anim-fade p-4">
        <div className="text-sm text-neutral-400">
          <span className="font-semibold text-accent">{q.opener}</span> açtı — sen{" "}
          <span className="font-semibold text-neutral-100">{q.position}</span>'desin.
        </div>
        <div className="mt-3 flex items-center gap-3">
          <HandGlyph code={q.hand} size="lg" />
          <div className="text-sm text-neutral-400">Elin. Kararın?</div>
        </div>
      </div>
      ...
      {answered && (
        <div className="flex flex-col gap-3">
          ...
          >
            {correct ? "✓ Doğru — " : `✗ Yanlış — sen ${ACTION_LABEL[chosen!]} dedin. `}
            Doğru cevap: <b>{ACTION_LABEL[q.correct]}</b>. {q.note}
          </div>

          <div className="card p-3">
            <div className="mb-2 text-sm font-semibold text-neutral-100">
              {q.position} · {q.opener} açılışına 3-bet aralığı
            </div>
