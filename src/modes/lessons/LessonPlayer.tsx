...
export function LessonPlayer({
  ...
  // rehberli anlatım durumu
  const [revealCount, setRevealCount] = useState(ALL); // kaç odak görünür (ALL = hepsi)
  const [activeFocus, setActiveFocus] = useState(-1); // o an konuşulan odak
  const [seg, setSeg] = useState({ i: 0, n: 0 }); // cümle ilerlemesi
  ...
  // Aktif odak değişince ekranda ortala (auto-scroll — göz takibi).
  useEffect(() => {
  ...
  if (!mod) {
    return (
      <div className="p-4">
        ...
        <p className="mt-4 text-neutral-400">Modül bulunamadı.</p>
      </div>
    );
  }
  ...
  // Bu slaytta odakların pozisyonları (render sırasıyla).
  ...
  const stop = () => {
    ...
    setSpeaking(false);
    setActiveFocus(-1);
    setRevealCount(ALL); // durunca her şey görünür kalsın
  };

  // Rehberli oynatma: her cümleyi seslendir, eşleşen odağı beliriş + vurgula.
  const start = async (from: number) => {
    ...
    for (let k = from; k < slides.length; k++) {
      ...
      for (let s = 0; s < segs.length; s++) {
        if (!playing.current) break;
        // sonraki cümleyi arka planda önden getir → boşluksuz akış
      ...
      setRevealCount(ALL); // slayt bitti → hepsi görünür
      setActiveFocus(-1);
  ...
  return (
    <div className="flex min-h-full flex-col">
      ...
      <div ref={bodyRef} className="flex-1 space-y-4 px-4 py-5">
        ...
        {/* oynatma kontrolleri — üstte, sabit */}
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            title="Slaytları otomatik ilerlet"
          >
            ⟳ Oto {auto ? "açık" : "kapalı"}
          </button>
        ...
        {/* canlı cümle ilerleme çubuğu */}
        {speaking && seg.n > 0 && (
          <div className="flex items-center gap-2">
            <span className="live-pulse text-xs text-accent">● anlatılıyor</span>
        ...
        {!speaker.supported && (
          <p className="text-xs text-neutral-500">Bu tarayıcı sesli anlatımı desteklemiyor.</p>
        )}

        {/* bullets — her biri bir odak */}
        {slide.bullets && (
  ...
  return (
    <div className="space-y-2">
      {parsed ? (
        ...
        <p className="text-sm text-neutral-500">Tablo yüklenemedi.</p>
      )}
