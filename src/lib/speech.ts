...
// Build-time bake: tüm ders anlatımları önceden public/tts/<key>.mp3'e basılır
// (scripts/bake-tts.mjs, AYNI hashKey). Canlıda proxy YOK → HD ses buradan gelir.
async function fetchStatic(key: string): Promise<Blob | null> {
  try {
    ...
    // 404 yerine HTML dönen host'lara karşı basit koruma
...
// HD ses blob'unu getir: IndexedDB → static bake → proxy (yalnız dev). Sonucu önbelleğe yazar.
async function loadAudio(text: string): Promise<Blob | null> {
  ...
  if (!blob) {
    try {
      ...
      /* proxy yok → null (Web Speech'e düşülür) */
    }
  }
...
class HybridSpeaker implements Speaker {
  ...
  async speak(text: string, rate = 1.0): Promise<void> {
    ...
    // HD: metni forSpeech'ten GEÇİRME (gerçek TTS terimleri düzgün okur).
    const blob = await loadAudio(text);
    if (!blob) return this.web.speak(text, rate); // ses bulunamadı → Web Speech
