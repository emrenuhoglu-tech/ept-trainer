...
// 🃏 Turn & River drill'i — postflop yarısı. Senaryolar Bölüm 6 (turn'de draw) ve Bölüm 11 (turn
// disiplini + river icrası) tablolarından gelir. Kitap boyutu (kalibre et) verdiği için yalnız
// YÖN sorulur; reveal, kaynak tabloyu gösterir ki "neden" kitabın kendi cevabı olsun.
...
export function Postflop() {
  ...
  // İlk soru mount'ta, render'da değil (record* yan etkileri + StrictMode çift sayımı).
  useLayoutEffect(() => {
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between text-sm">
        ...
        <span className="text-neutral-400">
          Oturum {score.ok}/{score.total}
        </span>
      </div>

      {!q && <div className="p-6 text-neutral-400">Postflop drill'i kurulamadı.</div>}
        ...
        <>
          <div key={`${q.kavram}-${score.total}`} className="card anim-fade p-4">
            <div className="mb-1 text-[11px] uppercase tracking-wide text-accent">
              {q.street === "turn" ? "Turn" : "River"} · Kitap {q.source}
            </div>
          ...
          {answered && (
            <div className="flex flex-col gap-3">
              ...
              >
                {correct ? "✓ Doğru — " : `✗ Yanlış — sen ${label(chosen!)} dedin. `}
                Doğru cevap: <b>{label(q.correct)}</b>. {q.note}
              </div>
              ...
              {q.catalog && (
                <div className="card p-3">
                  <div className="mb-2 text-sm font-semibold text-neutral-100">Kötü river kataloğu (11.4)</div>
