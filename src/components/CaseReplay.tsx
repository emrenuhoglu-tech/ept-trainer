...
// Vaka replayer: sokak sokak board kurulur, her sokakta kitaba dayalı bir not.
// Bilinmeyen kart (kitapta yok) "??" ile kapalı gösterilir — uydurma yok.
export interface ReplayStreet {
  name: string; // "Preflop" | "Flop" | "Turn" | "River"
  add?: string; // bu sokakta açılan kart(lar): "2c 4d 5s" | "??" | "6h"
  note: string; // kitaba dayalı açıklama
}
  ...
  return (
    <div className="card p-4">
      <div className="mb-3 flex items-center justify-between">
        <span className="text-sm font-semibold text-neutral-100">🎬 Vaka oynatıcı</span>
      ...
      <div className="mt-3 flex flex-wrap gap-4">
        <CardRow spec={data.hero} size="sm" label={data.heroLabel ?? "Sen"} />
        {data.villain && <CardRow spec={data.villain} size="sm" label={data.villainLabel ?? "Rakip"} />}
      </div>
