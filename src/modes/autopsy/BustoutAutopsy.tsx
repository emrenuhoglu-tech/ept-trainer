...
export function BustoutAutopsy({ onBack }: { onBack: () => void }) {
  ...
  // Tekrar oynatım: ilk tamamlanış işaretliyse karneye YAZMA — ezberlenmiş ikili
  // cevaplar 'kök-hata' streak'ini şişirir. Tekrarlar self-explain içindir.
    ...
    <div className="flex min-h-full flex-col gap-4 p-4">
      ...
      {replay && (
        <div className="rounded-lg bg-surface-2 px-3 py-2 text-xs text-neutral-400">
          Tekrar oynatım — puanlanmaz; kendine açıklamaya odaklan.
        </div>
      )}
...
// B11.0 "kalibre et: 3 elenme eline SPR biç" slotu. Sayılar EMRE'nin hafızasından
// girilir — uygulama hiçbir pot/stack UYDURMAZ, yalnız aritmetik (stack ÷ pot) yapar
// ve kitabın 11.0 bant tablosundan (READ-ONLY parse) eşleşen satırı gösterir.
interface SprRecord {
  id: string;
  pot: number;
  stack: number;
...
function SprEstimator({ hand }: { hand: Hand }) {
  ...
  const table = useMemo(() => tableFromSection("Bölüm 11", "11.0"), []);
  ...
  function compute() {
    ...
    recordResult({
      ...
      soru_ozeti: `SPR biç: ${hand.title}`,
      sonuc: "correct",
      not: `flop pot ${p}bb, kalan stack ${s}bb → SPR ${v}`,
    });
  }
  ...
  return (
    <div className="card p-3">
      <div className="text-xs uppercase tracking-wide text-neutral-500">
        SPR biç (B11.0 · isteğe bağlı)
      </div>
      <p className="mt-1 text-xs text-neutral-500">
        Hafızandan gir: floptaki pot ve arkanda kalan stack (bb). Hatırlamıyorsan boş bırak —
        sayı uydurma.
      </p>
      ...
      {spr !== null && (
        <div className="mt-3 text-sm">
          ...
          {row ? (
            ...
            <p className="mt-1 text-xs text-neutral-500">Bant tablosu yüklenemedi (B11.0).</p>
          )}
        </div>
      )}
    </div>
