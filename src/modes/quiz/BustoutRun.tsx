...
// Bustout Run — sudden-death: bir yanlış = run biter (turnuvadaki gibi).
// Run başına 1 "pas (fold)" hakkı — masada da fold = pas. En iyi run saklanır.
  ...
  if (over) {
    return (
      <div className="flex flex-col items-center gap-4 p-6 text-center">
        ...
        <p className="text-lg text-neutral-100">Run bitti — <b>{runLen}</b> doğru</p>
        <p className="text-sm text-neutral-500">En iyi: {best}</p>
        <div className="w-full rounded-xl bg-surface-1 px-4 py-3 text-left text-sm text-neutral-300">
          <b className="text-red-300">Bitiren karar:</b> {s.q}
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between text-sm">
        ...
        <span className="text-neutral-500">en iyi {best} · bir yanlış = biter</span>
      </div>
      ...
      {!answered && (
        ...
        >
          {skipUsed ? "Pas hakkı kullanıldı" : "🃏 Pas (fold) — run başına 1 hak"}
        </button>
      )}
